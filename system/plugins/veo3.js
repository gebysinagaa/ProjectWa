...
export const run = {
  ...
  category: 'ai',
  async: async (m, { kzm, text, Scraper, Utils, isPrefix, command }) => {
    ...
    if (!text) return kzm.reply(m.chat, Utils.example(isPrefix, command, prompt), m)
    ...
    if (/image\/(jpe?g|png)/.test(mime)) {
      ...
      if (!url) return kzm.reply(m.chat, global.status.wrong, m)
      imgUrls.push(url.data.url)
    }

    kzm.sendReact(m.chat, '🕒', m.key)
    ...
    app.run({
      ...
      .then(async (result) => {
        if (result?.data?.video_nowm_url) {
          await kzm.sendFile(m.chat, result.data.video_nowm_url, `${Date.now()}-veo3.mp4`, `🍟 *Process* : ${((new Date - old) * 1)} ms`, m)
        } else {
          kzm.reply(m.chat, `❌ Failed to get video URL.`, m)
        }
      })
      .catch(err => kzm.reply(m.chat, Utils.jsonFormat(err), m))
  },
  error: false,
  premium: true,
  restrict: true,
